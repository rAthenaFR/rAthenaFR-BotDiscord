...
impl RAthenaFrDatabase {
    ...
    async fn login_columns(&self) -> Result<AvailableColumns> {
        self.table_columns("login")
            .await?
            .ok_or_else(|| anyhow::anyhow!("La table `login` est absente."))
    }

    async fn ensure_login_column(&self, column_name: &str) -> Result<()> {
        let columns = self.login_columns().await?;
    ...
    async fn execute_account_i64_update(
        &self,
        sql: &str,
        value: i64,
        account_id: i64,
    ...
    async fn execute_account_text_update(
        &self,
        sql: &str,
        value: &str,
        account_id: i64,
    ...
    async fn execute_bound_account_update<'q>(
        &self,
